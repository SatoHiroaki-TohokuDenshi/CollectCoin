...
//初期化用変数
namespace Direct3D {
	HRESULT CreateDASC(HWND hWnd, int screenWidth, int screenHeight);
	HRESULT CreateRTV();
	HRESULT CreateDSV(int screenWidth, int screenHeight);
	void CreateDSS();
	void CreateBS();
...
HRESULT Direct3D::Initialize(HWND hWnd, int screenWidth, int screenHeight) {
	...
	hr = CreateDASC(hWnd, screenWidth, screenHeight);
	//失敗したら終了
	if (FAILED(hr)) {
		return hr;
	}
	...
	hr = CreateDSV(screenWidth, screenHeight);
	//失敗したら終了
	if (FAILED(hr)) {
		return hr;
	}
...
//シェーダー関連で必要なセット準備
void Direct3D::InitShaderBundle() {
...
//デバイス等の作成
HRESULT Direct3D::CreateDASC(HWND hWnd, int screenWidth, int screenHeight) {
	HRESULT hr{};
...
//深度ステンシルビューの作成
HRESULT Direct3D::CreateDSV(int screenWidth, int screenHeight) {
	HRESULT hr{};
...
void Direct3D::CreateShaderDebug() {
	DWORD vectorSize = sizeof(XMFLOAT3);
...
void Direct3D::CreateShaderBillBoard() {
	DWORD vectorSize = sizeof(XMFLOAT3);
